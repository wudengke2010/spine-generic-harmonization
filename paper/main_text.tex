...
All five methods reduced vendor $\eta^2$ by approximately three orders of magnitude, to ${\sim}10^{-4}$ (mean post-harmonisation $\eta^2$: ComBat $=4.1\times10^{-4}$, ComBat-joint $=4.9\times10^{-4}$, CovBat $=2.5\times10^{-4}$, RELIEF $=4.4\times10^{-4}$, LME/FE $=7.9\times10^{-4}$; Figure~2a,b, Table~2). For reference, the $\eta^2$ expected under the null hypothesis of no vendor effect is $(k-1)/(N-1)=2/187\approx0.011$; post-harmonisation values lie far below this chance level, so the effective reduction relative to chance expectation is 92.8--97.7\% across methods (LME/FE lowest at 92.8\%, CovBat highest at 97.7\%). Multivariate vendor structure became indistinguishable from chance (all PERMANOVA $p=1.0$ in both cohorts). These near-zero values should, however, be read as a process check rather than an independent validation: vendor is the harmonisation batch variable, and each method explicitly removes vendor location and scale, so a correctly implemented pipeline is expected to drive $\eta^2$ toward zero by construction. Machine-learnability (below) provides the stricter, non-degenerate criterion.

Covariance structure was displaced from the original to a comparable degree by all methods (Supplementary Fig.~S4). Expressed relative to the total Frobenius norm of the original cross-metric correlation matrix ($\|R_{\mathrm{orig}}\|_F=4.08$ in HC, $4.07$ in ALL), post-harmonisation correlation matrices lay 37.4--41.3\% away from the original in the HC cohort (ComBat 37.4\%, LME/FE 38.3\%, ComBat-joint 38.5\%, RELIEF 40.3\%, CovBat 41.3\%; corresponding range 39.8--43.0\% in ALL: ComBat 39.8\%, LME/FE 40.5\%, ComBat-joint 40.5\%, RELIEF 42.3\%, CovBat 43.0\%). No method therefore preserved the covariance structure in absolute terms. Between-method differences were an order of magnitude smaller than this shared displacement. CovBat and RELIEF yielded near-identical correlation matrices to each other (Frobenius distance 0.09 in HC, 0.08 in ALL), as did ComBat and ComBat-joint (0.11 in HC, 0.10 in ALL); the location--scale methods remained marginally closer to the original than the covariance-projection methods.
