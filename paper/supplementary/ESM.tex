...
\caption{Cross-metric covariance structure preservation. \textbf{(a)} Pearson correlation matrices of the eight biomarkers (HC cohort, $N=188$) before and after each method. \textbf{(b)} Frobenius distance between the correlation matrices of each pair of methods (lower = more similar covariance structure). Expressed relative to the total Frobenius norm of the original correlation matrix ($\|R_{\mathrm{orig}}\|_F=4.08$ in HC, $4.07$ in ALL), all methods displaced the covariance structure by 38.1--41.3\% in HC (ComBat 38.1\%, LME/FE 38.3\%, ComBat-joint 38.5\%, RELIEF 40.3\%, CovBat 41.3\%; 40.2--43.0\% in ALL), so covariance preservation holds only in relative terms. CovBat and RELIEF are the closest to each other (HC: 0.09, ALL: 0.08), indicating near-identical covariance structure despite the additional ICA round-trip in RELIEF. ComBat-joint and ComBat are also close (HC: 0.05, ALL: 0.04), showing that joint empirical Bayes has limited impact on the covariance structure beyond the per-metric ComBat baseline. The location--scale methods (ComBat HC: 1.55, ALL: 1.64; LME/FE HC: 1.56, ALL: 1.65) remain marginally closer to the original than the covariance-projection methods (CovBat HC: 1.69, ALL: 1.75; RELIEF HC: 1.65, ALL: 1.72).}
\end{figure}
