...
\subsection*{S6. Design C Subsample Analyses (Reduced Imbalance and Equal-n)}

To isolate the effect of vendor imbalance, two subsample designs were analysed in the HC cohort (20 random seeds each, all five methods refitted per subset using the same canonical implementations as the main analysis; subject-level preservation is reported as the pooled $r_{\text{pre,post}}$, the same metric as Table~2 of the main text). The reduced-imbalance design restricted Siemens to 40 randomly drawn subjects per seed while retaining all GE and Philips subjects (GE 25, Philips 33, Siemens 40; $n\approx98$ per subset; residual imbalance $\max/\min=1.60$ vs.\ $5.2$ in the full analysis). The true-equal-$n$ sensitivity design drew 25 subjects per vendor ($n=75$; imbalance ratio $=1.00$). Vendor $\eta^2$, PERMANOVA $R^2$, pooled and within-vendor subject-level preservation, and age-signal recovery were recomputed per seed.

In the reduced-imbalance design the pooled-$r$ preservation hierarchy was retained (LME/FE $r=0.779\pm0.011$, ComBat-joint $0.773\pm0.014$, RELIEF $0.760\pm0.015$, CovBat $0.749\pm0.016$, ComBat $0.745\pm0.015$; the only rank change vs.\ the full analysis was the adjacent exchange of ComBat and CovBat, a pair not significantly separated in the main analysis), and the true-equal-$n$ design gave concordant results (LME/FE $0.772\pm0.015$, ComBat-joint $0.768\pm0.016$, RELIEF $0.754\pm0.014$, CovBat $0.743\pm0.017$, ComBat $0.732\pm0.017$). Absolute values were lower than in the full cohort ($0.782$--$0.816$) because the subsamples carried a higher baseline vendor-variance fraction: the measured unharmonised $\eta^2$ was $0.374$ (range $0.343$--$0.401$ across seeds) in the reduced-imbalance design and $0.389$ (range $0.337$--$0.431$) in the true-equal-$n$ design, versus $0.315$ in the full analysis. Because pooled $r$ after a location--scale correction cannot exceed $\sqrt{1-\eta^2}$, these baselines imply ceilings of approximately $0.79$ and $0.78$; every per-method mean fell below its design's ceiling, and in a per-seed check the LME/FE pooled $r$ never exceeded the per-seed ceiling $\sqrt{1-\eta^2}$ in any of the 20 seeds of either design (smallest margins $0.007$ and $0.004$). Within-vendor subject rankings were nonetheless preserved near-perfectly by every method in both designs (mean within-vendor $r\geq0.97$), indicating that the pooled-$r$ gradient reflects differential removal of between-vendor variance rather than degradation of within-vendor subject rankings. Results are shown in Supplementary Fig.~S2.
...
\caption{Design C subsample analyses: reduced-imbalance and true-equal-n designs (HC cohort, 20 random seeds each; all methods refitted per subset). \textbf{(a)} Pooled subject-level preservation ($r_{\text{pre,post}}$, the main-text Table~2 metric): reduced-imbalance subsample (Siemens restricted to 40 subjects per seed, all GE and Philips subjects retained; $n\approx98$ per subset; imbalance ratio $1.60$) and true-equal-$n$ subsample (25 subjects per vendor, $n=75$, ratio $1.00$); dashed lines mark the full-cohort reference values. \textbf{(b)} Within-vendor subject-ranking preservation (Pearson $r$ computed within each vendor group; equal to 1 for pure location-shift methods by construction). \textbf{(c)} Vendor effect size ($\eta^2$) on logarithmic scale; the dashed line marks the unharmonised subsample baseline (mean $\eta^2\approx0.37$--$0.39$). The pooled-$r$ preservation hierarchy was retained in both designs (reduced imbalance: LME/FE $0.779\pm0.011$, ComBat-joint $0.773\pm0.014$, RELIEF $0.760\pm0.015$, CovBat $0.749\pm0.016$, ComBat $0.745\pm0.015$; equal-$n$: LME/FE $0.772\pm0.015$, ComBat-family $0.732$--$0.768$), with absolute values below the full cohort ($0.782$--$0.816$) because the subsamples carry a higher baseline vendor-variance fraction; within-vendor rankings were preserved near-perfectly by every method (mean $r\geq0.97$).}
\end{figure}
...
\caption{Cross-metric covariance structure preservation. \textbf{(a)} Pearson correlation matrices of the eight biomarkers (HC cohort, $N=188$) before and after each method. \textbf{(b)} Frobenius distance between the correlation matrices of each pair of methods (lower = more similar covariance structure). Expressed relative to the total Frobenius norm of the original correlation matrix ($\|R_{\mathrm{orig}}\|_F=4.08$ in HC, $4.07$ in ALL), all methods displaced the covariance structure by 37.4--41.3\% in HC (ComBat 37.4\%, LME/FE 38.3\%, ComBat-joint 38.5\%, RELIEF 40.3\%, CovBat 41.3\%; 39.8--43.0\% in ALL: ComBat 39.8\%, LME/FE 40.5\%, ComBat-joint 40.5\%, RELIEF 42.3\%, CovBat 43.0\%), so covariance preservation holds only in relative terms. CovBat and RELIEF are the closest to each other (HC: 0.09, ALL: 0.08), indicating near-identical covariance structure despite the additional ICA round-trip in RELIEF. ComBat-joint and ComBat are also close (HC: 0.11, ALL: 0.10), showing that joint empirical Bayes has limited impact on the covariance structure beyond the per-metric ComBat baseline. The location--scale methods (ComBat HC: 1.55, ALL: 1.64; LME/FE HC: 1.56, ALL: 1.65) remain marginally closer to the original than the covariance-projection methods (CovBat HC: 1.69, ALL: 1.75; RELIEF HC: 1.65, ALL: 1.72).}
\end{figure}
